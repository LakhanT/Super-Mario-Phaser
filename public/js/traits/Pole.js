...
export default class Pole extends Trait {
    ...
    addTraveller(pole, traveller) {
        ...
        poleTraveller.holding = true;
        poleTraveller.exiting = false;

        const player = traveller.traits.get(Player);
        if (player) {
            const span = Math.max(pole.size.y, 1);
            const fromTop = Math.max(0, traveller.bounds.top - pole.bounds.top);
            const ratio = 1 - fromTop / span;
            player.score += ratio > 0.85 ? 5000
                : ratio > 0.6 ? 2000
                : ratio > 0.4 ? 800
                : ratio > 0.15 ? 400
                : 100;
        ...
        for (const [traveller, state] of this.travellers.entries()) {
            if (!state.done) {
                ...
                if (traveller.bounds.bottom > state.goal.y) {
                    ...
                    poleTraveller.distance = 0;
                    poleTraveller.holding = false;
                    poleTraveller.exiting = true;
                    const go = traveller.traits.get(Go);
                    if (go) {
                        go.dir = 1;
                        go.heading = 1;
                    }
                }
            } else if (!state.finished && traveller.pos.x > pole.pos.x + 56) {
                state.finished = true;
                const poleTraveller = traveller.traits.get(PoleTraveller);
                poleTraveller.exiting = false;
                const go = traveller.traits.get(Go);
                ...
                level.events.emit(Level.EVENT_FINISH);
            }
        }
    }
}
